...
/** Show the color progression over the bottom edge of the board. */
function draw_merge_guide(game: Game, ctx: CanvasRenderingContext2D) {
    let height = 38;
    let top = game.ViewportHeight - height;
    let step = game.ViewportWidth / (COLORS.length + 1);
    let y = top + height / 2;
    let radius = Math.min(6, step / 4);
    ...
    for (let tier = 0; tier < COLORS.length; tier++) {
        let x = (tier + 0.5) * step;
        ...
        ctx.lineTo(x + step - radius - 3, y);
        ctx.lineTo(x + step - radius - 6, y - 3);
        ctx.moveTo(x + step - radius - 3, y);
        ctx.lineTo(x + step - radius - 6, y + 3);
        ctx.stroke();
    }
    ...
    ctx.fillText("★", (COLORS.length + 0.5) * step, y);
}
